...
Never embed a token in a remote URL: it persists in `.git/config` and can leak
through logs. Prefer the OS keychain or `gh auth setup-git`; use plaintext
`store` only with explicit user acceptance. Request minimum scopes and prefer
a fine-grained token scoped to the required repositories where supported.
...
```bash
# Verify read access only; this does not prove push permission.
git ls-remote https://github.com/<their-username>/<any-repo>.git
...
while true; do
  ...
  case "$POLL" in
    ...
    *) echo "Unexpected OAuth response; stop without printing its potentially sensitive body."; exit 1 ;;
  esac
done
```
...
> this), consider the file-store fallback below only with explicit user consent.
> It contains plaintext credentials: honor `GH_CONFIG_DIR` (or
> `${XDG_CONFIG_HOME:-$HOME/.config}/gh`), preserve existing accounts/settings,
> use restrictive permissions before writing, and never overwrite an existing
> `hosts.yml` wholesale. The minimal example is for a new, empty gh config only:
>
> ```bash
...
```bash
# Set GITHUB_SKILL_DIR to skill_view(name="github")'s returned skill_dir.
# Capture the result; running the extractor alone would print the credential.
GITHUB_TOKEN=$(python3 "${GITHUB_SKILL_DIR:?Set the installed github skill directory}/scripts/git-credential-token.py")
export GITHUB_TOKEN
```

### Helper: Detect Auth Method

Source `scripts/gh-env.sh` in Bash using the installed directory returned by
`skill_view`; do not construct a path to a former skill or another profile.
The helper tries authenticated `gh`, then `GH_TOKEN`/`GITHUB_TOKEN`, the selected
`${HERMES_HOME:-$HOME/.hermes}/.env`, then the exact-host credential-store parser.
It never sources `.env`, never prints credentials, validates REST auth with a
bounded `/user` request, and tolerates absent repos or credentials under
`set -euo pipefail`. A detected token alone is not proof of authentication.
The legacy `scripts/github-auth-gh-env.sh` forwards to this same implementation.

Maintenance verification: run `python3 "$GITHUB_SKILL_DIR/scripts/test-auth-offline.py"`
and `bash -n "$GITHUB_SKILL_DIR/scripts/gh-env.sh"`. The suite uses isolated
HOME/profile fixtures and stub `gh`, `git`, and `curl`; it does not verify live
GitHub credentials. Keep the credential parser compatible with macOS Python 3.9
and preserve its exact-host, percent-decoding and control-character safeguards.
REST headers in these files contain shell variables; tool displays may redact
them as `***`. Check byte-level equality before treating redaction as corruption.

Use this pattern at the start of any remote GitHub workflow:

```bash
# Set GITHUB_SKILL_DIR to skill_view(name="github")'s returned skill_dir.
source "${GITHUB_SKILL_DIR:?Set the installed github skill directory}/scripts/gh-env.sh"
# GH_AUTH_METHOD: gh, curl, or none. Stop remote operations on none.
# GH_USER and GH_OWNER/GH_REPO/GH_OWNER_REPO are populated when available.
```

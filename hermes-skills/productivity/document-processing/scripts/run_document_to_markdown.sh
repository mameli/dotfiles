...
PDF input is intentionally blocked; use scripts/run_pdf_to_markdown.sh.
...
if [[ "$LOWER_INPUT_PATH" == *.pdf ]]; then
  echo "PDF input is not supported by this wrapper. Use scripts/run_pdf_to_markdown.sh instead." >&2
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
RUNTIME_DIR="$SKILL_DIR/.runtime/office"
...
# uv environments may not include pip; repair existing environments too.
...
if [[ -n "$OUTPUT_PATH" ]]; then
  ...
  TMP_OUTPUT="$(mktemp "$(dirname "$OUTPUT_PATH")/.document-markdown.XXXXXX")"
else
  TMP_OUTPUT="$(mktemp)"
fi
cleanup() {
